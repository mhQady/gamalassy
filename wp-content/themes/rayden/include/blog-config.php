...
// pagination
if( !function_exists('rayden_pagination') ){

	function rayden_pagination( $current_query = null, $navigation_type = "blog-nav-minimal" ){
		...
		if( $pages > 1 )
		{
			...
			if( $navigation_type == "blog-nav-minimal" ){
				...
				if( get_previous_posts_link() ){
					...
					previous_posts_link( '<i class="fa fa-long-arrow-left" aria-hidden="true"></i>' );
				}
				else {
					
					echo '<div class="blog-prev-wrap">';
					echo '<div class="blog-prev">';
					echo '<i class="fa fa-long-arrow-left" aria-hidden="true"></i>';
				}
				echo '</div>';
				echo '</div>';
				?>
					<div class="blog-numbers">    
						<div class="blog-active"><?php echo wp_kses( $paged, 'rayden_allowed_html' ); ?></div>        
						<div class="blog-total"><?php  echo wp_kses( $current_query->max_num_pages, 'rayden_allowed_html' ); ?></div>    
					</div>    
				<?php
				if( get_next_posts_link('', $current_query->max_num_pages) ){
					...
					next_posts_link('<i class="fa fa-long-arrow-right" aria-hidden="true"></i>',  $current_query->max_num_pages);
				}
				else {
...
// comments
if( !function_exists('rayden_comment') ){

	function rayden_comment($comment, $args, $depth) {
		...
		echo '<span>';
		echo esc_html__('at', 'rayden') . ' ' . get_comment_time() . ', ' . get_comment_date() . ' - ';
		comment_reply_link(array_merge( $args, array('reply_text' => esc_html__('Reply', 'rayden'), 'before' => '', 'after' => '', 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '</span>';
		echo '</p>';
		
		echo '<div class="comment-text">';
		if ($comment->comment_approved == '0'){
			echo '<em>' . esc_html__("Your comment is awaiting moderation", 'rayden') . '</em><br />';
		}
		comment_text();
...
if( !function_exists('rayden_commentform_title') ){
	function rayden_commentform_title( $args ) {
...
add_filter( 'comment_form_defaults', 'rayden_commentform_title' );

// the caption displayed within single blog post hero pages
if( !function_exists('rayden_blog_post_hero_caption') ){

	function rayden_blog_post_hero_caption() {
