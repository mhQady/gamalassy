...
$rayden_max_items		= rayden_get_post_meta( RAYDEN_THEME_OPTIONS, get_the_ID(), 'rayden-opt-page-portfolio-mixed-items' );
if( empty($rayden_max_items) ){
	$rayden_max_items = 1000;
}
...
$rayden_content_position				= rayden_get_post_meta( RAYDEN_THEME_OPTIONS, get_the_ID(), 'rayden-opt-page-portfolio-mixed-content-position' );
		...
		<div id="main">
			...
			<div id="main-content">
				<div id="main-page-content">
					...
					<?php 
						if( $rayden_content_position == "before" ){
							the_content();
						} 
					?>
					...
					<div id="itemsWrapperLinks" class="portfolio-wrap <?php echo sanitize_html_class( $rayden_portfolio_layout_type ); ?> <?php echo sanitize_html_class( $rayden_portfolio_appear_effect_type ); ?> <?php echo sanitize_html_class( $rayden_portfolio_caption_type ); ?> <?php if( !rayden_get_theme_options('clapat_rayden_enable_ajax') ){ echo 'thumb-no-ajax'; } ?>">
						...
						<?php
							...
							$rayden_args = array(
										...
										'posts_per_page' => $rayden_max_items,
										 );
					...
					<?php 
						if( $rayden_content_position == "after" ){
							the_content();
						} 
					?>
