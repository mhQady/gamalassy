...
<body <?php body_class(); ?> data-primary-color="<?php echo esc_attr(rayden_get_theme_options('clapat_rayden_primary_color')); ?>">
	...
	<main>
		...
		<div class="cd-index cd-main-content">
			...
			$rayden_bknd_color_attribute = "#004E42";
			if ($rayden_bknd_color == "light-content") {

				$rayden_bknd_color_attribute = "#F5E3CB";
			}
			...
			<?php if (is_page_template('blog-page.php') || is_home() || is_archive() || is_search()) { ?>
				<!-- Page Content -->
				<div id="page-content" class="blog-template <?php echo sanitize_html_class($rayden_bknd_color);
															if (!rayden_get_theme_options('clapat_rayden_enable_magic_cursor')) {
																echo " magic-cursor-disabled";
															} ?> <?php echo esc_attr($rayden_page_content_classes); ?>" data-bgcolor="<?php echo esc_attr($rayden_bknd_color_attribute); ?>">
				<?php } else { ?>
					<!-- Page Content -->
					<div id="page-content" class="<?php echo sanitize_html_class($rayden_bknd_color);
													if (!rayden_get_theme_options('clapat_rayden_enable_magic_cursor')) {
														echo " magic-cursor-disabled";
													} ?> <?php echo esc_attr($rayden_page_content_classes); ?>" data-bgcolor="<?php echo esc_attr($rayden_bknd_color_attribute); ?>">
					<?php } ?>
